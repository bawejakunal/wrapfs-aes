...
#ifdef WRAPFS_CRYPTO
	char *key = NULL;
	if(cmd == 1)	//ioctl request number is 1 then treat as request to set key
	{
		...
		if(0 == memcmp(zero_key, key, KEYLEN))
			memcpy(WRAPFS_SB(file->f_dentry->d_sb)->key, key, KEYLEN+1);
		else
			calculate_md5(key,WRAPFS_SB(file->f_dentry->d_sb)->key, KEYLEN);
	}
#endif
