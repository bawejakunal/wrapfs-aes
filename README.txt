...
Once downloaded, copy the contents onf wrapfs folder to fs/wrapfs/ in kernel sources or put your own patch from http://wrapfs.filesystems.org/
...
A) Run a minimalist original Wrapfs implementation without any address space operations.
B) Run Wrapfs with address space operations.
C) Run Wrapfs with AES-CTR encryption using address space operations.

Compiling: 
Go to wrapfs folder
1) Give command "make" or "make all" if you want to use Wrapfs with option A or B i.e without encryption.
2) If you want to use Wrapfs with encryption give the command as "make CRYPTO+=-DWRAPFS_CRYPTO" which sets the WRAPFS_CRYPTO flag.

Insert module:
#insmod wrapfs.ko

Mounting:
1) To use Wrapfs with option A: mount -t wrapfs /mnt/lower /mnt/wrapfs
2) To use Wrapfs with option B or C: mount -t wrapfs -o mmap /mnt/lower /mnt/wrapfs --- It takes mmap option to specify whether address operations or not.

Use encryption:
In case you are using encryption you need to pass a passphrase to set the encryption key. Do that as follows:
	1) ./wrapfs_setkey -k passphrasevalue /mnt/wrapfs
